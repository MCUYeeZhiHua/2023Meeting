  ...
  for (int L=12-1; L>=0; L--) {
    for (int i=0; i<256; i+=1) {
      for (int j=0; j<256; j+=1) {
        int ii =i*256+j;
        float x=j*(NX/256), y=i*(NY/256), z=L*(NZ/12);
        StrandTrans[L][ii][0] = Interpolate(x, y, z); //總共31個頂點
        for (int s=0; s<segmentN; s++) { //迴圈計算30個頂點
          x+=LayerX[L][ii][s]*len;
          y+=LayerY[L][ii][s]*len;
          z+=LayerZ[L][ii][s]*len;
        ...
        if (mousePressed) { //照著梯度的垂直方向畫出來的線，剛好和線的方向有關
          ...
          if (LL[L]) {
            ...
            for (int s=0; s<segmentN; s++) { //照原始照片，加上色彩
              ...
              vertex(x, y);
              myStroke(img.pixels[ii],StrandTrans[L][ii][s+1]*1, 255*transparent); //L=Layers, ii=256*256, s=segmentN
